...
export default function ActivateDeactivateCell(props) {
        ...
        if (typeof props.axiosFunction === "function" && props.idKey) {
            ...
            try {
                ...
                if (res?.status === 200) {
                    setIsActive(active);
                    props.setData((prev) => {
                        if (typeof prev.currentPage === "number" ||
                            ...
                            try {
                                props
                                    ...
                                    .then((res) => {
                                    if (res?.status === 200) {
                                        dispatch(setTableAlert({
                                            ...tableAlert,
                                            show: true,
                                            highLight: "Success ",
                                            text: `s/no. ${id} is ${active === 1 ? "Activate" : "Deactivate"} successfully applied.`,
                                            type: "green",
                                        }));
                                ...
                                if (err?.response?.status === 400) {
                                    dispatch(setTableAlert({
                                        ...
                                        text: `s/no. ${id} is ${isActive === 1 ? "Activate" : "Deactivate"} not applied. ${err?.response?.data?.message}`,
                                        type: "red",
                                    }));
                                }
                            }
    ...
    return (<button disabled={props.axiosFunction ? false : true} onClick={() => onActive()} className={"w-fit py-1 px-1.5 rounded-full flex space-x-1 items-center bg-opacity-10 ".concat(isActive === 1
