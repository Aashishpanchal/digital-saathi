...
export default function SubCategories() {
    ...
    const onSubCategoriesDelete = async () => {
        if (value) {
            ...
            try {
                ...
                const res = await subCategories("delete", {
                    params: category_id,
                });
                if (res.status === 200) {
    ...
    const onSubCategoryEdit = (values) => navigate(`${values.category_id}`);
    const columns = React.useMemo(() => [
        ...
        },
        {
            ...
            filter: "equals",
            Cell: (cell) => (<ActiveDeactivateCell cell={cell} idKey="category_id" setData={setData} axiosFunction={subCategories} postfix={`&category_id=${parent_category_id}`}/>),
            extraProps: {
                columnStyle: {
                    width: "250px",
                    textAlign: "center",
    ...
    return (<AdminContainer>
      <MainContainer heading={`${category_name} / Subcategories`}>
        ...
        {loading ? (<div className="flex flex-col justify-center items-center space-y-3 mt-4">
          ...
          </div>) : data.totalItems ? (<Table columns={columns} data={getData} showPagination page={page} changePage={(page) => setPage(page)} totalEntries={data.totalItems} totalPages={data.totalPages - 1} entriesPerPage={10}/>) : (<div className="flex flex-col space-y-4 justify-center items-center font-bold">
            <FcDeleteDatabase size={100}/>
            <h2 className="text-lg">Sorry Data Not Available</h2>
          </div>)}
      </MainContainer>
      <DeleteModal show={deleteModalShow} onClose={() => setDeleteModalShow(false)} onClickNo={() => setDeleteModalShow(false)} onClickYes={onSubCategoriesDelete}/>
    </AdminContainer>);
}
