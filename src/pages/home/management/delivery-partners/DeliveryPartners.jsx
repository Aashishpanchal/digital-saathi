...
export default function DeliveryPartners() {
    ...
    const onDeliveryPartnerDelete = async () => {
        if (value) {
            ...
            try {
                ...
                const res = await deliveryPartners("delete", {
                    params: partner_id,
                });
                if (res.status === 200) {
    ...
    const onDeliveryPartnerEdit = (value) => navigate(`/management/delivery-partners/${value.partner_id}`);
    const onNext = (value) => navigate(`/management/delivery-partners/${value.partner_id}/retailer`);
    const columns = React.useMemo(() => [
        ...
        },
        {
            ...
            Cell: (cell) => (<ActiveDeactivateCell cell={cell} idKey="partner_id" axiosFunction={deliveryPartners}/>),
        },
        {
            Header: "Partner Name",
        ...
        },
        {
            ...
            Cell: (cell) => (<TableActionsCell cell={cell} onDelete={async (value, setDeleteLoading) => {
                ...
                }} onEdit={onDeliveryPartnerEdit} onNext={onNext}/>),
        },
    ], []);
    ...
    return (<AdminContainer>
      <MainContainer heading="Delivery Partners">
        <div className="mb-4">
          <Button onClick={onNew} icon={<TbTruckDelivery size={18}/>} color="dark">
            New
          </Button>
        </div>
      ...
      <DeleteModal show={deleteModalShow} onClose={() => setDeleteModalShow(false)} onClickNo={() => setDeleteModalShow(false)} onClickYes={onDeliveryPartnerDelete}/>
    </AdminContainer>);
}
