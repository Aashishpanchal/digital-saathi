...
export default function Products() {
    ...
    const onCategoryDelete = async () => {
        if (value) {
            ...
            try {
                ...
                const res = await shopProducts("delete", {
                    params: sku_id,
                });
                if (res.status === 200) {
                    await onProductsGet();
    ...
    const onShopProductEdit = (value) => navigate(`/management/products/${value.sku_id}`);
    const onProductWeightPrice = (value) => navigate(`/management/products/${value.sku_id}/product-weight-price/${encodeURI(value.sku_name)}`);
    ...
    const columns = React.useMemo(() => [
        ...
        },
        {
            ...
            filter: "equals",
            Cell: (cell) => (<ActiveDeactivateCell cell={cell} idKey="sku_id" axiosFunction={shopProducts} setData={setData}/>),
            extraProps: {
                columnStyle: {
                    width: "250px",
                    textAlign: "center",
        ...
        },
        {
            Header: "Focus SKU",
            accessor: "focus_sku",
            Cell: (cell) => (<FocusSKUCell cell={cell} idKey="sku_id" setData={setData}/>),
        },
        {
            Header: "Action",
            Cell: (cell) => (<TableActionsCell cell={cell} onDelete={async (value, setInnerLoading) => {
                    setDeleteModalShow(true);
                    setValue({
                        sku_id: value.sku_id,
                        setInnerLoading,
                    });
                }} onEdit={onShopProductEdit} onWeightPrice={onProductWeightPrice}/>),
        },
    ], []);
    ...
    return (<AdminContainer>
      <MainContainer heading="Products">
        <div className="mb-4">
          <Button onClick={onNew} icon={<MdProductionQuantityLimits size={18}/>} color="dark">
            New
          </Button>
        </div>
        {loading ? (<div className="flex flex-col justify-center items-center space-y-3 mt-4">
          ...
          </div>) : data.products ? (<Table columns={columns} data={getData} showPagination page={page} changePage={(page) => setPage(page)} totalEntries={data.totalItems} totalPages={data.totalPages - 1} entriesPerPage={10}/>) : (<div className="flex flex-col space-y-4 justify-center items-center font-bold">
