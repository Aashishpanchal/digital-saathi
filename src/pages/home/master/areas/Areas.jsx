...
export default function Areas() {
    ...
    const onShopAreasDelete = async () => {
        if (value) {
            ...
            try {
                ...
                const res = await shopAreas("delete", {
                    params: area_id,
                });
                if (res.status === 200) {
    ...
    const onShopAreasEdit = (values) => navigate(`/masters/areas/${values.area_id}`);
    ...
    return (<AdminContainer>
      <MainContainer heading="Areas">
        <div className="mb-4">
          <Button onClick={onNew} icon={<TbSquareAsterisk size={18}/>} color="dark">
            New
          </Button>
        </div>
      ...
      <DeleteModal show={deleteModalShow} onClose={() => setDeleteModalShow(false)} onClickNo={() => setDeleteModalShow(false)} onClickYes={onShopAreasDelete}/>
    </AdminContainer>);
}
